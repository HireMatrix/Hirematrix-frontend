...
export const checkAuth = createAsyncThunk('auth/checkAuth', async (_, { rejectWithValue }) => {
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/auth/check-auth`, {
            method: 'GET',
...
export const verifyEmail = createAsyncThunk('auth/verifyEmail', async(verificationDetails, {rejectWithValue}) => {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/auth/verify-email/${token}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
...
export const resetPassword = createAsyncThunk('auth/resetPassword', async (resetPassDetails, {rejectWithValue}) => {
    ...
    try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/v1/auth/reset-password/${token}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
